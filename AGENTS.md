...
Localized strings are managed directly in `Azkar/Resources/Localizable.xcstrings`.
...
Use native string-catalog lookups such as `NSLocalizedString("common.done", comment: "")` or SwiftUI localized APIs, and keep using `Asset.someImage` for asset catalogs.
